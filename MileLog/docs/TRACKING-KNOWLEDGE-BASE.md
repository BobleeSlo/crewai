...
**Last updated:** 2026-07-21 (Phase 15, see `TRACKING-AUDIT-2026-07-21.md`) — added §4 rows on the Always→While-Using silent permission downgrade and the app-suspended-for-hours `endedAt` backdating fix. Sections 2, 3, 5, and 6 were last substantively rewritten 2026-07-03 (after the Phase 14 trip-end redesign AND its round-3 adversarial-review follow-up fixes, see `TRACKING-AUDIT-2026-07-03.md`) — the design they describe (pure movement-based trip ending, Bluetooth as identification-only) is a deliberate reversal of the "BT keep-alive" policy this file previously documented. If you are reading a stale copy or a cached summary of this file, discard anything about "keep alive while Bluetooth connected" — that policy caused a real-world ~19.5-hour, 131.7 km false trip and has been removed. Movement evidence for the end decision comes from **two independent, BT-free signals** — GPS (`lastMovementAt`) and CoreMotion (`MotionVerifier.lastAutomotiveActivityAt`) — not GPS alone; see §3 and §4.
...
**Current rule (Phase 14, 2026-07-03, refined by round-3 adversarial review the same day):** a trip ends when `stationaryTimeoutMinutes` have elapsed since `lastMovementAt` (GPS-based movement) — UNLESS CoreMotion has reported "automotive" activity within the last `motionSignalFreshnessSeconds` (120s), in which case ending is deferred (bounded by `gpsBlackoutOverrideCapMinutes` = 45 min so a stuck sensor still can't run a trip forever). Both signals — GPS and CoreMotion — are **movement evidence**, gathered independently of each other. Bluetooth state has **zero** influence on whether a trip is considered ongoing, full stop. Bluetooth's only job anywhere in this system is **identification** — deciding which vehicle (private car, business car) a trip belongs to, both at start (fast path) and mid-trip (`checkVehicleSwitch`'s "adopt" and "switch" branches).

Why two movement signals instead of one: GPS alone can't distinguish "genuinely parked" from "GPS signal lost while still driving" (a tunnel, underground garage, or an ordinary iOS background-suspension gap) — both simply stop producing location callbacks, so `lastMovementAt` goes stale identically in either case. This gap was found by the round-3 adversarial review, which pointed out that reintroducing Bluetooth to resolve it (Phase 12's approach) would undo the very fix the user asked for. CoreMotion's accelerometer-based activity classification runs on-device without needing GPS or Bluetooth, so it's the correct second signal: if the phone is truly stationary, CoreMotion reports `.stationary` almost immediately (accelerometer, not satellite-dependent); if the car is still moving, it keeps reporting `.automotive` regardless of GPS/BT signal. See §4.
...
**Rule for anyone touching this file:** if you're tempted to add a new "end the trip if X minutes have passed" check anywhere other than inside `auditActiveTrip()`, don't. Every stationary-based decision must go through that one function, which is the *only* place that decides trip lifetime. It is allowed to consult multiple **movement** signals (GPS timing, CoreMotion activity) — that's not a violation of this rule — but must never consult Bluetooth for this purpose. Two earlier mistakes of introducing a second, competing stationary check outside `auditActiveTrip()` were found and removed in the 2026-06-28 audit (`checkStationary()`, and `locationManagerDidPauseLocationUpdates` ending trips directly) — **watch for a third instance if new iOS delegate callbacks are ever added.**

## 4. Known iOS/CoreLocation/AVAudioSession quirks that shape this code

| Quirk | Where it matters | How the code handles it |
|---|---|---|
| iOS suspends background apps for minutes at a time, even with location permission "Always" — and if the phone is **genuinely stationary** (no displacement to trigger any wake at all), this can stretch to **hours**, not just minutes (confirmed in the field 2026-07-21: a trip that stopped moving 22s after starting wasn't formally closed out until 4h47m later, when the next real drive's significant-location-change finally woke the app) | GPS updates and `Timer`s (audit timer, verification deadline) can all silently stop firing, for as long as there's nothing to wake the app | Verification checks wall-clock elapsed time on every update it *does* receive, not just its own `Timer`; `restoreActiveTripIfAny()` resumes GPS + audit on relaunch after a kill. **`endTrip(reason:at:)` (Phase 15) backdates a stationary-timeout ending's `endedAt` to `trip.lastMovementAt`** — the true last-movement instant — instead of `Date()`, so a trip that only finishes closing out hours late doesn't record a multi-hour duration for what was really a brief stop |
| iOS can **silently revert location authorization from "Always" to "While Using"** for an app that mostly runs in the background without being brought to the foreground (Apple's own "background location use" reminder — if not actively answered, it can auto-downgrade). Confirmed recurring in the field: happened at least twice within 8 days (2026-07-21 log) | Background auto-detect depends entirely on Always — `startMonitoringSignificantLocationChanges()` cannot wake a backgrounded/terminated app under While Using. A silent downgrade makes the app go completely quiet with zero indication, for as long as nobody happens to open Settings/the Record tab | `locationManagerDidChangeAuthorization` (Phase 15) detects the transition (previous status was `.authorizedAlways`, new status isn't) and immediately fires a local push notification (`NotificationManager.sendPermissionDowngradedNotification`) telling the user to re-grant it — nothing can prevent the OS from downgrading it, but the app no longer stays silent about it |
| `CLLocationManager` does not call `didUpdateLocations` while the device hasn't moved past `distanceFilter` | A truly parked phone generates ~zero location callbacks — but so does a phone in a GPS dead zone (tunnel, underground garage) while still genuinely moving. GPS-arrival timing alone **cannot tell these two cases apart** (confirmed by the round-3 adversarial review, which found this let a real drive auto-end mid-tunnel under the original Phase 14 logic) | The per-update `checkStationary()` removal (2026-06-28 audit) is still safe. But `auditActiveTrip()`'s stationary-timeout check now also consults `MotionVerifier.lastAutomotiveActivityAt` (CoreMotion, GPS-independent) before ending — see §3 — specifically to cover the "still moving, GPS just went dark" case that GPS timing alone can't detect |
| `CMMotionActivityManager` classifies activity (automotive/stationary/walking/etc.) from onboard accelerometer + co-processor data — it does not need GPS or Bluetooth signal, so it keeps working in a tunnel or underground garage | The only available signal that can disambiguate "genuinely parked" from "GPS lost while still driving," now that Bluetooth can't be used for this (§3) | `MotionVerifier` was extended (round-3 review, 2026-07-03) to run for a trip's **full duration** (previously it stopped once trip-start verification passed) and expose `lastAutomotiveActivityAt`, a live-refreshed timestamp `auditActiveTrip()` checks before ending a GPS-stationary trip |
...
| `stationaryTimeoutMinutes` (Settings-configurable) | user-set, single-digit minutes by default | **The primary trip-end criterion.** Elapsed minutes since `lastMovementAt` past this value ends the trip, unless overridden by a fresh CoreMotion automotive reading (see next two rows). See §3. |
| `motionSignalFreshnessSeconds` | 120s | How recent CoreMotion's last "automotive" classification must be to defer ending a GPS-stationary trip — covers a tunnel/GPS-dead-zone while still driving |
| `gpsBlackoutOverrideCapMinutes` | 45 min | Absolute bound on the CoreMotion override above — even with automotive activity still being reported, a trip stops being kept alive past this much GPS-stationary time, so a stuck/misclassifying sensor can't run a trip forever. (Movement-signal-based, not Bluetooth-based — does not reintroduce the Phase 12 problem.) |
...
Four places in this codebase have a direct line to what appears on a tax/reimbursement report, and deserve the most scrutiny in any future change:
...
2. **`distanceKm`** — silently lost distance (e.g. a flapped-away middle segment discarded as noise, or a GPS-blackout segment that auto-ended before the CoreMotion fix) understates a legitimate business trip.
...
4. **`startLat`/`startLng`** (added round 3, 2026-07-03) — feeds `TripClassifier`'s home/work proximity check and the reverse-geocoded start address. `tryMergeWithRecentTrip` used to silently substitute the previous leg's END point here, which could misclassify a genuine commute as business/private after one ordinary stop. Any change to how a merged/resumed trip's start point is derived must be checked against this.

Any change to `checkVehicleSwitch`, `tryMergeWithRecentTrip`, `matchVehicle`, or `endTrip` should be checked against all four before merging.
